...
import net.neilcsmith.praxis.video.java.VideoConstants.Blend;
...
//                image.getWidth(), image.getHeight()));
    ...
    public void blendMode(Blend blend) {
        blendMode(blend, blend.getOpacity());
    }

    public void blendMode(Blend blend, double opacity) {
        this.blendMode = blend.getMode();
        if (opacity < 0) {
            opacity = 0;
        } else if (opacity > 1) {
            opacity = 1;
    ...
    public void image(PImage src, double x, double y, double w, double h,
        ...
        if (transform != null) {
            ...
            image.process(transformBlit, src);
        } else {
            if (iw == srcW && ih == srcH) {
                blit.setX(ix)
                        .setY(iy)
