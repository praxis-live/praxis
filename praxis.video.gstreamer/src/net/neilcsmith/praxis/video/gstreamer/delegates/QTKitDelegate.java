 ...
 * @author Neil C Smith
 */
public class QTKitDelegate extends AbstractGstDelegate {

    private final static String SRC_TYPE = "qtkitvideosrc";
    private final static String DEFAULT_CAPS = "video/x-raw-rgb";
    private String capsString;
    private int deviceIdx;

    private QTKitDelegate(int deviceIdx, String caps) {
    ...
    public static QTKitDelegate create(URI resource) {
        if (!resource.isAbsolute()) {
            throw new IllegalArgumentException();
        }
        if (!"qtkit".equals(resource.getScheme())) {
            throw new IllegalArgumentException();
        }
        int idx = getSrcIndex(resource);
        String caps = getCapsString(resource);
        return new QTKitDelegate(idx, caps);
    }
