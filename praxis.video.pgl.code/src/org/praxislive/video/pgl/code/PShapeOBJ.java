 ...
 *
 */
class PShapeOBJ extends PShape {
    ...
    PShapeOBJ(URI uri) {
        ...
        parseOBJ(uri, faces, materials, coords, normals, texcoords);
    ...
    static protected void parseOBJ(URI uri,
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if (parts.length > 0) {
                    if (parts[0].equals("v")) {
                        ...
                        if (parts[1] != null) {
                            String fn = parts[1];
                            URI mtlURI = uri.resolve(fn);
//                            BufferedReader mreader = parent.createReader(fn);
//                            if (mreader != null) {
                            parseMTL(mtlURI, materials, mtlTable);
//                                mreader.close();
//                        }
                        }
    ...
    static protected void parseMTL(URI uri,
            ArrayList<OBJMaterial> materials,
...
//                            }
                            URI imageURI = uri.resolve(texname);
                            currentMtl.kdMap = loadImage(imageURI);
    ...
    private static PImage loadImage(URI uri) throws Exception {
        Image awtImage = ImageIO.read(uri.toURL());
