...
import net.neilcsmith.ripl.render.opengl.internal.ShaderProgram;
...
public class GLFilter extends AbstractExecutionContextComponent {
    ...
    private void init() {
        try {
            CompositeDelegator cmp = new CompositeDelegator(1);
            cmp.setDelegate(delegate);
            cmp.addSource(new Placeholder());
            Placeholder in = new Placeholder();
            cmp.addSource(in);
            registerPort(Port.IN, new DefaultVideoInputPort(this, in));
            registerPort(Port.OUT, new DefaultVideoOutputPort(this, cmp));
...
//        }
    ...
    private class GLDelegate implements CompositeDelegate {
        ...
        public void process(Surface surface, Surface... sources) {
            if (surface instanceof GLSurface) {
                ...
                GLSurface src = (GLSurface) sources[0];
                GLSurface dst = (GLSurface) surface;
